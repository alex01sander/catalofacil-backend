...
router.get('/', async (req, res) => {
    try {
        ...
        const totalDomains = await prisma_1.default.domain_owners.count();
        // Vendas do mês
        ...
        const recentDomains = await prisma_1.default.domain_owners.findMany({
            include: {
                users: {
                    select: {
                        id: true,
...
router.put('/domains/:id', async (req, res) => {
    try {
        ...
        if (domain && domain !== existingDomain.domain) {
            const duplicateDomain = await prisma_1.default.domain_owners.findUnique({
                where: { domain }
            });
            if (duplicateDomain) {
                return res.status(409).json({ error: 'Domínio já cadastrado' });
...
router.post('/register-domain-user', async (req, res) => {
    try {
        const { domain, user_email, user_password, user_role = 'admin', domain_type = 'domain', user_id = null } = req.body;
...
router.get('/reports/sales', async (req, res) => {
    try {
        ...
        const summary = await prisma_1.default.sales.aggregate({
            where,
            _sum: {
                total_price: true,
                quantity: true
