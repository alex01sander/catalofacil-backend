...
describe('Orders Routes', () => {
  ...
  afterAll(async () => {
    ...
    await prisma.credit_transactions.deleteMany({ where: { user_id: userId } });
    await prisma.credit_accounts.deleteMany({ where: { user_id: userId } });
    await prisma.sales.deleteMany({ where: { user_id: userId } });
    await prisma.cash_flow.deleteMany({ where: { user_id: userId } });
    await prisma.order_items.deleteMany({ where: { order_id: { in: await prisma.orders.findMany({ where: { store_owner_id: userId }, select: { id: true } }).then(orders => orders.map(o => o.id)) } } });
    await prisma.orders.deleteMany({ where: { store_owner_id: userId } });
    await prisma.customers.deleteMany({ where: { user_id: userId } });
  ...
  describe('GET /orders/:id/processing-status', () => {
    ...
    it('deve retornar erro 403 para pedido de outro usuário', async () => {
      ...
      const otherOrder = await prisma.orders.create({
        data: {
      ...
      // Limpar dados de teste
      await prisma.orders.delete({ where: { id: otherOrder.id } });
  ...
  describe('PUT /orders/:id', () => {
    ...
    it('deve processar pedido quando status muda para aceito', async () => {
      ...
      const cashFlow = await prisma.cash_flow.findFirst({
        where: {
          user_id: userId,
          description: {
            contains: `Pedido #${order.id.substring(0, 8)}`
  ...
  describe('Processamento de pedidos com crédito', () => {
    it('deve criar conta de crédito quando pedido tem telefone do cliente', async () => {
      ...
      const creditAccount = await prisma.credit_accounts.findFirst({
        where: {
          customer_phone: '11999999996',
          user_id: userId
        }
      ...
      const creditTransaction = await prisma.credit_transactions.findFirst({
        where: {
