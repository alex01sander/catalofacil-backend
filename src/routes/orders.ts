...
async function processOrderAcceptance(orderId: string, userId: string) {
  ...
  try {
    ...
    const vendaExistente = await prisma.sales.findFirst({
      where: {
        user_id: userId,
        product_name: {
...
router.get('/:id/processing-status', authenticateJWT, async (req, res) => {
  const parse = idParamSchema.safeParse(req.params);
  if (!parse.success) {
    return res.status(400).json({ error: 'Parâmetro inválido', details: parse.error.issues });
  }
  ...
  try {
    ...
    const vendasRelacionadas = await prisma.sales.findMany({
      where: {
        user_id: req.user.id,
        product_name: {
          contains: `Pedido #${pedido.id.substring(0, 8)}`
    ...
    const entradaCaixa = await prisma.cash_flow.findFirst({
      where: {
        user_id: req.user.id,
        description: `Venda - Pedido #${pedido.id.substring(0, 8)}`
      },
...
router.put('/:id', authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    const pedidoAtual = await prisma.orders.findUnique({
      where: { id: req.params.id },
...
router.post('/:id/reprocess', authenticateJWT, async (req, res) => {
  const parse = idParamSchema.safeParse(req.params);
  if (!parse.success) {
    return res.status(400).json({ error: 'Parâmetro inválido', details: parse.error.issues });
  }
